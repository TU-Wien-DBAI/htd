...
struct htd::TreeDecompositionViaSeparatorAlgorithm::Implementation
{
	...
	*/
	Implementation(const htd::LibraryInstance * const manager) : managementInstance_(manager), separatorAlgorithm_(manager->highestDegreeSeparatorAlgorithmFactory().createInstance()), labelingFunctions_(), postProcessingOperations_(), computeInducedEdges_(true), numberOfSteps_(4), sizeLimit_(6), algorithmType_(1), criteriaType_(3)
	{ 
...
void htd::TreeDecompositionViaSeparatorAlgorithm::Implementation::computeMutableDecompositionSeparators(htd::IMutableTreeDecomposition * ret, const htd::IMultiHypergraph & graph, const htd::IPreprocessedGraph & preprocessedGraph, vertex_t index, std::vector<vertex_t> oldSeparator, int counter) const
{
	...
	if (index != NULL)
	{
		...
		if (separators != oldSeparator && separators.size()>0 )
		{
